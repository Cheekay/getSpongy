...
describe('toggleUpvote', () => {
  ...
  it('adds upvote when not yet voted (returns voted: true, incremented count)', async () => {
    ...
    mockServiceClient.from
      .mockReturnValueOnce(makeQuery({ error: null })) // insert upvote
      .mockReturnValueOnce(makeQuery({ error: null })) // update upvote_count
  ...
  it('removes upvote when already voted (returns voted: false, decremented count)', async () => {
    ...
    mockServiceClient.from
      .mockReturnValueOnce(makeQuery({ error: null })) // delete upvote
      .mockReturnValueOnce(makeQuery({ error: null })) // update upvote_count
